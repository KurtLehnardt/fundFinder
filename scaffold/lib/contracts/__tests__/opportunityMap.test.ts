 ...
 * in data/precomputed.json must still validate whether or not it carries the
 * tag. (Maps regenerated by the current `buildOpportunityMap` DO stamp
 * `version`, since §3.6 made the builder write it on every live map; older
 * frozen maps predate it. Both must validate.) This is the CON-01 escalation
 * guard turned into a test.
 */
...
test("a cached map with no version tag validates (version is optional / additive)", () => {
  // The tag is optional/additive: a map WITHOUT it must validate and parse with
  // `version === undefined`. Derive an explicitly-unstamped map by stripping any
  // version the current builder now stamps, so this asserts schema optionality
  // rather than depending on whether the frozen fixture predates the tag.
  const { version: _drop, ...unstamped } = precomputed[0].map as { version?: unknown };
  const parsed = OpportunityMapSchema.parse(unstamped);
